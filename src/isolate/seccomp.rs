...
pub fn apply_seccomp(profile: &str) -> Result<()> {
    ...
    let program = match profile {
        ...
        _ => anyhow::bail!(
            "Unknown seccomp profile '{}'; supported profiles are: strict, minimal, default",
            profile
        ),
    };
...
fn build_minimal() -> Result<BpfProgram> {
    let allowed: &[i64] = &[
        ...
        // Signals
        nix::libc::SYS_rt_sigaction, nix::libc::SYS_rt_sigprocmask,
        nix::libc::SYS_rt_sigreturn, nix::libc::SYS_rt_sigsuspend,
        nix::libc::SYS_kill, nix::libc::SYS_tgkill,
        // Filesystem
        ...
        nix::libc::SYS_getrusage, nix::libc::SYS_umask,
        ...
        nix::libc::SYS_timerfd_create, nix::libc::SYS_timerfd_settime,
        ...
        nix::libc::SYS_memfd_create, nix::libc::SYS_copy_file_range,
    ];
...
mod tests {
    ...
    #[test]
    fn unknown_profile_fails_before_filter_installation() {
        let err = apply_seccomp("typo-profile")
            .expect_err("unknown seccomp profile must fail closed");
        let message = err.to_string();
        assert!(message.contains("Unknown seccomp profile"));
        assert!(message.contains("strict, minimal, default"));
    }
}
